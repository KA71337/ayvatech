...
// Accessible custom language dropdown used by the public header.
const languageDropdown = document.querySelector('.language-dropdown');
if (languageDropdown) {
  const trigger = languageDropdown.querySelector('.language-dropdown-trigger');
  const list = languageDropdown.querySelector('.language-dropdown-menu');
  const options = [...languageDropdown.querySelectorAll('.language-dropdown-option')];
  const closeLanguages = (restoreFocus = false) => {
    trigger.setAttribute('aria-expanded', 'false');
  ...
  trigger.addEventListener('click', () => trigger.getAttribute('aria-expanded') === 'true' ? closeLanguages() : openLanguages());
  trigger.addEventListener('keydown', event => {
    if (['ArrowDown', 'ArrowUp'].includes(event.key)) { event.preventDefault(); openLanguages(); }
  });
  list.addEventListener('keydown', event => {
    const current = options.indexOf(document.activeElement);
    if (event.key === 'Escape') { event.preventDefault(); closeLanguages(true); return; }
  ...
  options.forEach(option => option.addEventListener('click', () => {
    try { localStorage.setItem('ayva.lang', option.lang); } catch {}
    closeLanguages();
  }));
  document.addEventListener('pointerdown', event => { if (!languageDropdown.contains(event.target)) closeLanguages(); });
  document.addEventListener('keydown', event => { if (event.key === 'Escape' && !list.hidden) closeLanguages(true); });
}
