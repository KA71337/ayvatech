...
export function requireAdmin(req,res,next){if(!authConfigured()||!req.session.admin){if(req.originalUrl.startsWith('/api/'))return res.status(401).json({error:'Unauthorized'});return res.redirect('/admin/login');}next();}
