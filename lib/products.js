...
const localizedText = max => z.object({ az: z.string().trim().min(1).max(max), ru: z.string().max(max).default(''), en: z.string().max(max).default('') });
const mediaPath = z.string().regex(/^\/media\/[a-zA-Z0-9_-]+\.(webp|jpg|png)$/);
export const imageSchema = z.object({ src: mediaPath, original: mediaPath, sourceUrl: z.string().url().optional(), width: z.number().int().positive().max(30000), height: z.number().int().positive().max(30000), variants: z.array(z.object({width:z.number().int().positive(),src:mediaPath})).max(5) });
export const productSchema = z.object({
  ...
  sourceUpdatedAt:z.string().optional(), importedAt:z.string().optional(), sourceHash:z.string().optional()
