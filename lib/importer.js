...
export function mergeImported(existing, incoming) {
  ...
  for (const product of incoming) {
    const keys = [product.sourceId, product.sourceUrl].filter(Boolean);
    if (!keys.length) throw new Error('Imported product needs a source identity');
    if (keys.some(key => seen.has(key))) { stats.duplicates++; continue; }
    keys.forEach(key => seen.add(key));
    const index = result.findIndex(p => (product.sourceId && p.sourceId === product.sourceId) || (product.sourceUrl && p.sourceUrl === product.sourceUrl));
    if (index < 0) { result.push(product); stats.added++; continue; }
    const old = result[index];
    if (old.sourceHash === product.sourceHash) {
      if (old.sourceUpdatedAt === product.sourceUpdatedAt) { stats.skipped++; continue; }
      // A listing bump is not a content change: retain the administrator's edits.
      result[index] = { ...old, sourceUpdatedAt: product.sourceUpdatedAt, importedAt: product.importedAt };
    ...
    result[index] = { ...product, id: old.id, slug: old.slug, status: old.status,
      ...
      category: { ...old.category, az: product.category.az } };
    stats.updated++;
  }
...
export async function importTap() {
  ...
  if (process.env.DATA_DIR && path.resolve(process.env.DATA_DIR) !== path.resolve('data')) throw new Error('Importer currently requires DATA_DIR=data');
  const release = await acquireWriteLock('data');
  try {
    ...
    // Validate and normalize source offsets before comparison and persistence.
    const merged = mergeImported(existing.map(product => productSchema.parse(product)), incoming.map(product => productSchema.parse(product)));
    merged.products = merged.products.map(product => productSchema.parse(product));
    for (const field of ['id', 'slug', 'sourceId', 'sourceUrl']) {
      const values = merged.products.map(p => p[field]).filter(Boolean);
      if (new Set(values).size !== values.length) throw new Error(`Duplicate ${field}; refusing write`);
    }
